...
export class ExercisesService {
  ...
  private buildJudgeTemplates(dto: PracticeEvaluationDto): JudgeTemplate[] {
    ...
    if (title.includes('basic html structure') || topic === 'HTML & Semantics') {
      ...
      addAny(
        ...
        isVi ? 'Dùng ít nhất một semantic tag' : 'Use at least one semantic tag',
      ...
      addAny(
        ...
        isVi ? 'Xử lý responsive hoặc positioning' : 'Handle responsive layout or positioning',
        '`@media`, `position: sticky`, `position: fixed`, hoặc `grid-template-columns`',
        [/@media/i, /position\s*:\s*sticky/i, /position\s*:\s*fixed/i, /grid-template-columns\s*:/i],
        isVi ? 'Đã có xử lý layout nâng cao.' : 'Advanced layout handling is present.',
        isVi
      ...
      addAny(
        ...
        [/aria-live=/i, /role=["']alert["']/i, /aria-invalid=/i, /aria-describedby=/i],
      ...
      addAny(
        ...
        [/\btabindex=/i, /\bkeydown\b/i, /\bEscape\b/i, /role=["']dialog["']/i, /aria-modal=/i],
      ...
      addAny(
        ...
        isVi ? 'Xử lý nhánh response hoặc flow chính' : 'Handle response branching or the main flow',
        '`if`, `switch`, `return`, `status`, hoặc `response` shape',
        [/\bif\s*\(/i, /\bswitch\s*\(/i, /\breturn\b/i, /\bstatus\b/i, /\bresponse\b/i],
      ...
      addAny(
        ...
        isVi ? 'Dùng primitive phù hợp với chủ đề' : 'Use topic-specific primitives',
        isVi
        ...
        isVi ? 'Đã có primitive theo domain.' : 'Domain primitives are present.',
        isVi
      ...
      addAny(
        ...
        [/\bdata\b/i, /\berror\b/i, /\bheaders\b/i, /\bbody\b/i, /\bidempot/i, /\btraceId\b/i],
      ...
      addAny(
        ...
        isVi ? 'Có primitive chính cho lời giải' : 'Use the main implementation primitive',
        '`function`, `async`, `useState`, `useEffect`, `describe`, hoặc type/interface',
        [/\bfunction\b/i, /\basync\b/i, /\buseState\b/i, /\buseEffect\b/i, /\bdescribe\s*\(/i, /\binterface\b/i, /\btype\b/i],
        isVi ? 'Đã có primitive chính.' : 'Core primitives are present.',
        isVi ? 'Code vẫn còn quá trống.' : 'The implementation is still too empty.'
      );
      addAny(
        'client-structure',
        isVi ? 'Readable structure' : 'Readable structure',
        isVi ? 'Giữ cấu trúc lời giải dễ đọc' : 'Keep the implementation readable',
        '`return`, helper, comment, selector, validation, hoặc assertion',
        [/\breturn\b/i, /\/\//, /#/, /\bselector\b/i, /\bvalidate\b/i, /\bexpect\s*\(/i],
      ...
      addAny(
        ...
        isVi ? 'Đã có xử lý phân tích dữ liệu.' : 'Data analysis logic is present.',
        isVi
    ...
    if (templates.length === 0) {
      ...
      addAny(
        ...
        isVi ? 'Có `return`, helper, hoặc comment' : 'Use `return`, helpers, or comments',
      ...
      addAny(
        ...
        isVi ? 'Chưa thấy tín hiệu bám topic.' : 'Not enough topic-specific cues yet.'
      );
    }
  ...
  private buildHiddenJudgeTemplates(dto: PracticeEvaluationDto): JudgeTemplate[] {
    const isVi = dto.locale === 'vi';
    const topic = dto.topic;
    ...
    if (topic === 'HTML & Semantics') {
      pushAny(
        'hidden-html-lang',
        isVi ? 'Hidden: khai báo document language' : 'Hidden: declare document language',
        isVi ? 'Kiểm tra cấu hình document nâng cao' : 'Check advanced document configuration',
        '`<html lang="...">` hoặc `<meta charset>`',
        [/<html[^>]+lang=["'][^"']+["']/i, /<meta[^>]+charset=/i],
        isVi ? 'Đã có cấu hình document tốt hơn.' : 'Document configuration looks solid.',
        isVi
      ...
      pushAny(
        ...
        [/\bminmax\s*\(/i, /\boverflow\s*:/i, /\bmax-width\s*:/i, /\bwidth\s*:\s*100%/i],
        isVi ? 'Đã có tín hiệu layout bền hơn.' : 'Layout resilience signals are present.',
        isVi
  ...
  private toDbSubmissionStatus(status: SubmissionStatusLabel): DbSubmissionStatus {
    switch (status) {
      case 'Accepted':
